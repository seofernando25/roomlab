...
try {
  ...
  const search=catalogue.locator('input[type=search]');await search.fill('Club Chair');await catalogue.locator('.object-card').first().click();
  await game(alice).locator('.catalogue-open').click();
  ...
  const inventoryAfterPlace=await api<{items:{prototypeId:string;state:string}[]}>(alice,'/api/inventory');
  if(!inventoryAfterPlace.items.some(i=>i.prototypeId==='chair'&&i.state==='placed'))errors.push('inventory: placed chair was not reserved');
  ...
  const dragDestination=await findHover(alice,'walk',h=>h.cell!==chairOnAlice.cell&&h.cell!==seatedCell);
  ...
  await alice.mouse.move(chairOnAlice.x,chairOnAlice.y);await alice.mouse.down();
  await alice.mouse.move((chairOnAlice.x+dragDestination.x)/2,(chairOnAlice.y+dragDestination.y)/2,{steps:5});
  await waitFor(async()=> (await canvasData(bob)).remoteManipulations==='1',5_000,'remote live furniture manipulation');
  if((await canvasData(bob)).playerPose!=='sit')errors.push('seating: visitor stood while chair was live-dragged');
  await alice.mouse.move(dragDestination.x,dragDestination.y,{steps:6});await alice.mouse.up();
  await waitFor(async()=> (await canvasData(bob)).remoteManipulations==='0',5_000,'manipulation commit');
  await waitFor(async()=>{const d=await canvasData(bob);return d.playerPose==='sit'&&d.playerCell!==seatedCell;},8_000,'seated actor authority following committed chair');

  // Picking up a seat while somebody references it is rejected locally and authoritatively.
  const selectedTitle=game(alice).locator('.selection-title');
  ...
  // Alice lists an inventory item; Bob buys that exact item instance.
  ...
  const listButton=items.locator('button').filter({hasText:'List for trade'}).first();await listButton.click();
  await waitFor(async()=> (await api<{listings:any[]}>(alice,'/api/market/listings')).listings.some(l=>l.sellerUsername===aliceName),5_000,'market listing');
  ...
  console.log(JSON.stringify({ok:true,users:[aliceName,bobName],roomPath,screenshots:['artifacts/online-lobby.png','artifacts/online-multiplayer-room.png','artifacts/online-shop.png','artifacts/online-friends.png']},null,2));
} finally {
