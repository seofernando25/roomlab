...
export class InventoryPage extends LitElement {
  ...
  private card(item:InventoryItemDto,canList:boolean){const def=getCatalogueObject(item.prototypeId);return html`<article class="card item"><div class="preview"><catalogue-object-preview prototype-id=${item.prototypeId}></catalogue-object-preview></div><div><h3>${def.label}</h3><div class="meta"><span class="badge">${stateLabel(item)}</span></div>${canList?html`<div class="actions"><input class="price" type="number" min="1" max="1000000" value="25" aria-label="Listing price"><button class="secondary" @click=${(event:Event)=>this.listItem(item,event)}>List for trade</button></div>`:null}</div></article>`;}
