...
export function createBlockStepsVisual(): THREE.Group {
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    const tread = new THREE.Mesh(new THREE.BoxGeometry(WIDTH, height, depth + 0.025), material.clone());
...
export function createGlassStairsVisual(): THREE.Group {
  ...
  for (let i = 0; i < 5; i += 1) {
    const t = i / 4;
    const tread = new THREE.Mesh(new THREE.BoxGeometry(WIDTH, 0.055, 0.20), glassMaterial());
    tread.position.set(0, 0.06 + t * RISE, -RUN / 2 + t * RUN);
    group.add(tread);
  }
  const postMaterial = toon(0x8ea5a9, 'metal');
  for (const x of [-0.34, 0.34]) {
    const post = new THREE.Mesh(new THREE.BoxGeometry(0.045, RISE + 0.30, 0.045), postMaterial.clone());
...
export function createMetalStairsVisual(): THREE.Group {
  ...
  for (let i = 0; i < 5; i += 1) {
    const t = i / 4;
    const tread = new THREE.Mesh(new THREE.BoxGeometry(WIDTH, 0.07, 0.18), treadMaterial.clone());
    tread.position.set(0, 0.055 + t * RISE, -RUN / 2 + t * RUN);
    group.add(tread);
  }
  for (const x of [-0.34, 0.34]) {
    const stringer = new THREE.Mesh(new THREE.BoxGeometry(0.065, 0.075, 1.03), frameMaterial.clone());
...
export function createMetalRampVisual(): THREE.Group {
  ...
  const ramp = new THREE.Mesh(new THREE.BoxGeometry(WIDTH, 0.085, length), toon(palette.metal, 'metal'));
  ramp.position.set(0, RISE / 2 + 0.035, 0);
  ramp.rotation.x = angle;
  group.add(ramp);
  for (const x of [-0.36, 0.36]) {
    const rail = new THREE.Mesh(new THREE.BoxGeometry(0.045, 0.045, length), toon(0x455c63, 'metal'));
