...
Room discovery is **snapshot + invalidation**, not polling. `rooms-page` and the in-room browser fetch the appropriate scoped `/api/rooms` snapshot when opened or searched, then keep an authenticated `/ws/directory` connection for lightweight `rooms-changed` invalidations. Room creation/settings and live join/leave presence publish an invalidation; clients coalesce those events and refresh the snapshot they are already authorized to see. The directory WebSocket deliberately carries no room collection itself, so Popular/Friends/Mine/Recent filtering and access rules remain on the HTTP boundary.

Inside a room, persistent app navigation is a compact procedural **Three.js dock** rather than text or Unicode navigation chrome. Its five modeled affordances are Lobby, Rooms, Shop, Inventory, and Profile; hover/focus lifts and turns the corresponding model. The Rooms action opens a lightweight live room list without leaving the current room. The room identity itself is a small collapsible bottom-right card containing only the room name and creator; owner settings live in that card. Internal topology information such as floor-tile counts is not player-facing.

The room browser currently uses a cheap schematic preview by design. A future true room thumbnail should come from a dedicated read-only preview projection/cache regenerated after meaningful edits; listing a room must never join its simulation or fetch/render every full authoritative room snapshot merely to paint a Navigator row.
...
The desktop smoke exercises camera modes, plain `-`/`=` keyboard zoom, floor hover, click-to-walk, authored walking, click-to-sit, Play/Edit switching, the horizontally scrolling Catalogue tray, the continuous Placement plane, floating-slab drag preview/commit, manual wall lines, standalone square Material Studio preview/presets/saved patterns, stack movement, panning, adding objects, rotation, and WebGL sizing. Browser console/page errors fail the smoke pass. The mobile smoke additionally verifies real touch scrolling on the landing/lobby, compact Shop rows, the five-action Three.js room dock, bottom-right room identity, in-room room-browser fit, non-overlapping chat/navigation chrome, a 44px placement-Y control, full-width Material Studio scrolling and touch sizes, square material preview geometry, tap-to-walk, one-finger camera panning, pinch zoom including the wider portrait zoom-out range, browser-zoom suppression, and full-viewport canvas behavior. The two-browser online acceptance flow verifies room-directory WebSocket invalidation with **no timed room polling**, live room-browser presence, exact-item placement/pickup/Marketplace transfer, chat replication/fade, continuous remote actor poses, no redundant inventory polling, automatic chair-under-actor seating, same-seat/manipulation command dedupe, seated live manipulation, rights, presence, friends, and restart/reconnect.
