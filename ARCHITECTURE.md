...
Room **discovery** is a separate realtime seam from a joined room session. Durable/filterable room collections remain authenticated HTTP snapshots; `/ws/directory` carries only revisioned invalidation signals for room metadata and live occupancy changes. A client reconnect or `ready` message reconciles by requesting the current scoped snapshot. Do not push a universal room list over the socket: Mine/Friends/Recent/search are user-specific queries, and keeping authorization/filtering at the snapshot boundary prevents accidental directory leakage.

The in-room navigation dock is presentation, not simulation. Procedural Three.js models describe the five navigation affordances, while DOM hit targets emit semantic app-navigation actions. Neither `RoomScene` nor the dock owns routing/account logic. Likewise, the simple in-room room browser consumes room-summary DTOs; it must not join other room simulations or materialize authoritative room worlds to make list thumbnails. A future rendered room preview belongs behind a dedicated read-only preview projection/cache.
